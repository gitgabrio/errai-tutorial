...
public class ContactListPage {
  ...
  /**
   * This is an Errai UI event handler. The element for which this handler is registered is in this class's HTML
  ...
  /**
   * Called for single-click events on the {@code tutorial-page} element
   */
  @EventHandler("tutorial-page")
  public void onTutorialPageClick(final @ForEvent("click") MouseEvent event) {
    navigation.goTo("TutorialPage");
  }
  ...
  /**
   * This is an Errai UI native event handler. The element for which this handler is registered is in this class's HTML
   ...
   * {@link ContactDisplay#onDoubleClick(MouseEvent)}, in order to display the
   * modal form for editing a contact.
   */
  public void editComponent(final @Observes @Click(Click.Type.DOUBLE) ContactDisplay component) {
    selectComponent(component);
    editModel(component.getValue());
  }

  /**
   * This method observes CDI events fired locally by {@link ContactDisplay#onClick(MouseEvent)} in order to highlight a
   * {@link ContactDisplay} when it is clicked.
   */
  public void selectComponent(final @Observes @Click(Click.Type.SINGLE) ContactDisplay component) {
   ...
   */
  public void onRemoteUpdated(final @Observes @Operation(Operation.OperationType.UPDATE) ContactOperation contactOperation) {
    if (sourceIsNotThisClient(contactOperation)) {
   ...
   * browser sessions.
   */
  public void onRemoteDelete(final @Observes @Operation(Operation.OperationType.DELETE) Long id) {
  ...
  /**
   * If the parameter is true then this displays a form for editing (with a delete button). Otherwise show a form for
